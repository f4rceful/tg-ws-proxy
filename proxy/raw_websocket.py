...
class RawWebSocket:
    ...
    async def send_batch(self, parts: List[bytes]):
        if self._closed:
            raise ConnectionError("WebSocket closed")
        # Optimization: Batch all frames into a single write call
        # This significantly reduces system calls and context switches,
        # especially important for asyncio performance under load.
        payload = b''.join(
