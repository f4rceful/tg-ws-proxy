...
class RawWebSocket:
    ...
    async def send_batch(self, parts: List[bytes]):
        if self._closed:
            raise ConnectionError("WebSocket closed")
        # ⚡ Bolt Optimization: Batch asyncio stream writes.
        # Joining bytes into a single payload reduces I/O scheduling
        # overheads and context switches compared to looping `writer.write()`.
        batch_payload = b''.join(
            self._build_frame(self.OP_BINARY, part, mask=True) for part in parts
        )
        if batch_payload:
            self.writer.write(batch_payload)
        await self.writer.drain()
