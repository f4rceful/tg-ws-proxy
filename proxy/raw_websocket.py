...
class RawWebSocket:
    ...
    async def send_batch(self, parts: List[bytes]):
        ...
        # ⚡ Bolt Optimization: Batch payload into a single write call
        # to reduce system calls and TCP fragmentation.
        batched_payload = b''.join(
            self._build_frame(self.OP_BINARY, part, mask=True) for part in parts
        )
        if batched_payload:
            self.writer.write(batched_payload)
